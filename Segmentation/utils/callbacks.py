...
class EvalCallback():
    ...
    def on_epoch_end(self, epoch, model_eval):
        if epoch % self.period == 0 and self.eval_flag:
            self.net    = model_eval
            gt_dir      = os.path.join(self.dataset_path, "MSRS/vi/SegmentationClass/")
            ...
            for image_id in tqdm(self.image_ids):
                vi_image_path  = os.path.join(self.dataset_path, "MSRS/vi/JPEGImages/"+image_id+".png")
                vi_image       = Image.open(vi_image_path)
                ir_image_path  = os.path.join(self.dataset_path, "MSRS/ir/JPEGImages/"+image_id+".png")
                ir_image       = Image.open(ir_image_path)
                image       = self.get_miou_png(vi_image, ir_image)
